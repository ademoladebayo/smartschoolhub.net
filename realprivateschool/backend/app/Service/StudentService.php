...
class StudentService
{
    ...
    public function getResult(Request $request)
    {
        ...
        $no_student = DB::select('select count(distinct student_id) as no_student from subject_registration where class_id ="' . $result->class_id . '" and session ="' . $request->session . '" and term ="' . $request->term . '"')[0]->no_student;
