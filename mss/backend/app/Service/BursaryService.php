...
class BursaryService
{
    ...
    public function getPayableForClass(String $class_id, String $session, String  $term)
    {
        ...
        $fees =  FeeModel::orWhere('class', $class_id)->orWhere('class', $class_sector)->orWhere('class', 'ALL STUDENT')->orWhere("type", "COMPULSORY")->where('session', $session)->where('term', $term)->get();
        $expected_amount = 0;
        foreach ($fees as $fee) {
